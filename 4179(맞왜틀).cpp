...
* 지훈이의 좌표는 1개만 주어진다고 지문에 쓰여있지만 불은 그렇지 않기 때문에 여러개가 올 수도 있다.
* 불이 퍼지지 못하거나 없는 경우도 있을 수 있다. 그럴 경우 불의 가중치는 0으로 고정되어 지훈 가중치보다 작아
* 현재 로직에선 지훈이가 움직이지 못하는 예외가 생긴다. 그렇기 때문에 불의 방문배열을 모두 INF로 초기화 하고
* 불의 좌표는 1로 초기화하여 진행하여 예외처리를 해준다.
* 
* if(fire_visited[ny][nx]) continue는 초기화값인 INF까지 continue하게 되므로 != INF로 방문처리를 해결한다.
* fire_visited != INF 는 방문한적이 있다면으로 해석된다.
*/
